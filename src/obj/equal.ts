...
const _deepEqualArr = (arrs: unknown[][]): boolean => {
...
const _deepEqualObj = (objs: object[]): boolean => {
...
const shallowEqual = (arr: unknown[]): boolean => {
...
const deepEqual = (args: unknown[]): boolean => {
  ...
  if (isArr) {
    return _deepEqualArr(args as unknown[][]);
  }

  return _deepEqualObj(args as object[]);
};
